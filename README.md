...
基于 Three.js 和 TypeScript 的网页大逃杀原型。项目包含 50 人对局, 小队 AI, 空降, 枪械配件, 游泳, 载具, 动态天气, 轰炸区, 区域资源和多层建筑。
...
- `/?test=1&scenario=maptour&region=mistwood&weather=fog&time=6.2`: 林地生态, 低空薄雾和清晨远景巡查
- `/?test=1&scenario=maptour&region=tideharbor&weather=rain&time=8`: 雨天岸线, 水面浅滩和海岸细节巡查
...
垂直切片精修版进一步增加窗洞进深, 法式阳台, 双层檐口, 老虎窗, 侧烟囱, 非盒体市场货物和完整餐厨卧室分区. 全局表面材质会随天气同步湿润暗部与粗糙度, 建筑和道具在降雨时不再保持晴天质感.

角色, 动画和镜头重制采用简洁统一的方块人比例, 肘膝关节参与陆地步态, 游泳, 空降, 翻越, 击倒和载具坐姿. 全流程固定第一人称视角, 步行, 跳伞, 游泳和驾驶均无第三人称切换入口. 镜头使用帧率稳定的落地弹簧, 起停惯性和交互注视, ADS 会主动收敛运动量以保持瞄准稳定.

地形与自然环境重制使用湿沙, 岸沙, 草甸, 林地, 旱草和裸岩六层生态权重统一驱动地表颜色与自然物分布. 林下增加蕨类, 落叶和倒木, 草甸增加野花群落, 岩体增加同色碎石坡, 河海岸线增加动态芦苇, 卵石和克制的泡沫水线. 水面补充浅滩色, 交叉波纹和雨滴涟漪, 地图外缘使用雾化多峰山脊建立远景层次, 天气系统同步控制地表湿润, 水面和低空薄雾.

地图与建筑资产化使用 `src/assetcatalog.ts` 统一登记建筑原型, 结构模块, 六区立面套件, 道路桥梁, 自然物, 战术掩体和区域地标. 建筑与地图生成器共享同一份表面材质规格, 并在测试场景数据中公开实际资产种类和实例数量, 便于检查资产缺失, 重复配置和内容预算回退.
